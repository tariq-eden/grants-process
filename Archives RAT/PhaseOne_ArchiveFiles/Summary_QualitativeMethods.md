...
In this context, our data included the text collected through the free-text responses that participants provided along with their numerical estimates about the likely replicability of specific research claims (on the repliCATS platform variation on the [IDEA protocol](https://doi.org/10.1111/2041-210X.12857)). This large data set was supplemented by interviews with a sub-set of the participants in repliCATS workshops. More detail on the data-collection process has been documented by our Elicitation Team. The following summary is provided as an overview of our qualitative approach to analysing this textual data during Phase 1 of the repliCATS project (Feb 2019 - Dec 2020).

While overlapping in practice, the qualitative and quantifiable distinction remains an influential way of differentiating between research involving non-numerical data (qualitative) such as text and images, and approaches that focus on data that can be handled numerically (quantitative)(Schwandt 2001; Vogt and Johnson 2016). While studies are sometimes considered qualitative simply due to involving non-numerical data, there is a wide range of analytic approaches within the qualitative methods literature. For example, within the social sciences it is common to focus on the frequency of various analytic categories (including term use or instances of types-of-responses). Other qualitative approaches, especially within the humanities, extend beyond the frequency of terms to examine the contextualised content of textual data (including adapting hermeneutic approaches to interpreting texts or following phenomenological traditions that emphasise descriptive accounts of subjective experiences).[^1]

In this project, we adopted techniques for identifying the appearance of content-types within text using a set of analytic categories (‘codes’) that goes beyond calculating the frequency of the application of such codes to interpret the underlying content of that text within the relevant context (Hsieh and Shannon 2005). We incorporated these contextualised content-analysis techniques for systematically interpreting textual data by developing rules for multiple analysts to consistently categorise content within passages of textual data in ways that supports convergent analyses of those texts. In interpreting the implicit meaning of texts, we followed existing guidelines by combining provisional coding for predefined grammatical and elemental codes with an exploratory stage of indexing additional content of relevance to the research questions (Saldaña 2016). This process went beyond providing instructions for identifying the appearance of terms (e.g., plausible) in the text, to specify synonymous terms and guide interpretation of implicit references to the category of analysis (e.g., content relevant to the category of plausibility might include texts such as ‘reason to believe’, ‘seemed reasonable’, and so on). This type of structured approach aligns with a process for qualitative content-analysis that has been described as “a research technique for making replicable and valid inferences from [a set of] text to their content” (Popping 2000, 7).

This analytic process was formalised by developing a codebook specifying the inclusion and exclusion criteria for those codes most relevant to our research questions. This codebook emerged through an iterative process where several different analysts independently analysed subsections of the data and then compared which codes each applied to the same sections of text. This process helped to ensure the stability of these codes in terms of both consistent application by multiple analysts (inter-coder agreement, ICA) and for reliably reflecting the content of the textual data collected at an appropriate level of complexity (inter-coder reliability, ICR). For example, multiple analysts used a key version of the codebook to index the same subset of the texts and then met resolve ambiguities suggested by ICA gaps suggested by calculating initial ICR of each code. These ICR results contributed to reviewing the convergence of textual interpretation between analysts. In addition, when this process included analysts naive to the codebook development, ICR calculations provided an indication of which analytic categories required further calibration and which could function as a reliable tool interpreting the text in relation to our research questions for multiple analysts.

The value of transparent process and the potential of re-analysing textual data is well-recognised within qualitative research practices even though it is not relevant for all studies (Wästerfors, Åkerström, and Jacobsson 2014; Dag Stenvoll and Peter Svensson 2011). Therefore, in addition to calibrating our codebook to support multiple analysts converging upon consistent interpretations of the text, we also documented the decisions made during the codebook development and further analysis. This includes the current documentation on how the textual data was indexed, as well as the archived version of our codebook, to support re-analysis by those interested in our research questions. However, given the amount and richness of the textual data collected, there would also be value in re-examining the data with different analytic categories and research questions entirely. Either way, by documenting the development of our analytic categories, we hope that any additional analyses of this textual data will have the benefit of potential cross-references with the analysis already conducted.

To this end, the following offers a brief summaries of key steps in the development and implementation of the qualitative analysis practices from Phase 1 of the repliCATS project. Additional details are available in the documentation archived for this project.

## Data Collection Process

Using a [technical implementation of the IDEA Protocol](https://scholarspace.manoa.hawaii.edu/handle/10125/70666), we used the 'repliCATS platform' to elicite two rounds of predictions about the likely replicability. In both rounds, free-text boxes were provided for participants to explain their understanding of the claim and justify the numerical ratings of the likely replicability of a claim using the free-text boxes for questions [1,3](https://github.com/metamelb-repliCATS/RAT/blob/drafts/Archives%20RAT/PhaseOne_ArchiveFiles/Example%20Documentation/1st%20Platform%20Screen%20Shot%202019-09-09%20at%209.08.45%20am.png) and [4](https://github.com/metamelb-repliCATS/RAT/blob/drafts/Archives%20RAT/PhaseOne_ArchiveFiles/Example%20Documentation/2nd%20Platform%20Screen%20Shot%202019-09-09%20at%209.08.51%20am.png)). Between rounds, participants were also able to comment on the round-1 justifications that other participants had provided on the platform. The majority of our qualitative data from Phase 1 was collected through these free-text justifications and comments on the repliCATS platform. This data was supplemented by data collected when groups met in person and facilitator-notes and audio recording of group discussions were recorded. In addition, we interviewed a subset of participants about their own individual reflections of experiences during the assessment of specific claims following the SIPS2019 workshop and the SIPS2020 workshop (although, due to delays with transcription, these interviews were not analysed during Phase 1 and will remain with the data set for further consideration in Phase 2).

## Data Preparation

The data exported from the platform was prepared by our data-management team as a set of files that could be uploaded into NVivo (our Computer-Aided Qualitative Data Analysis Software (CAQDAS)).[^2] While this data-preparation process evolved over the course of the project, each batch of claims was prepared for analysis in the form of four types of excel files (also see the data [pipeline overview](https://github.com/metamelb-repliCATS/aggreCAT)):

1. Platform Textual Responses (Justifications and Comments): each row functions as a unit of textual data (a justification or an associated comment), with a column for each of the characteristics of those responses (including a column for linking that data to the other file types). For example see, an [anonymised version of this file type, titled *BatchName_Justification_Comments*](https://github.com/metamelb-repliCATS/RAT/blob/drafts/Archives%20RAT/PhaseOne_ArchiveFiles/Example%20Documentation/BatchName_justifications_comments_2020-09-01.xlsx)

2. Platform Numerical Responses (Ratings): with one row for each participant per round per claim, columns were included for each question as well as for additional characteristics of those responses (including a column for linking that data to the other file types). For example see, an [anonymised version of this file type, titled *BatchName_Ratings_by_claim*](https://github.com/metamelb-repliCATS/RAT/blob/drafts/Archives%20RAT/PhaseOne_ArchiveFiles/Example%20Documentation/BatchName_ratings_by_claim_2020-09-01.xlsxs)

3. Claim Metadata: with one row per claim, columns were included for each of the characteristics of that claim (based on COS metadata for each claim: Claim id; journal; COS journal discipline; Web of science discipline). For example see, an [anonymised version of this file type, titled *BatchName_Claim_Metadata*](https://github.com/metamelb-repliCATS/RAT/blob/drafts/Archives%20RAT/PhaseOne_ArchiveFiles/Example%20Documentation/BatchName_claim_metadata_2020-09-01.xlsx)

4. Participant Attributes: with one row per participant, and columns for each of the demographic questions asked and each quiz-question answered. For example see, an [anonymised version of this file type, titled Demographics_anonymised](https://github.com/metamelb-repliCATS/RAT/blob/drafts/Archives%20RAT/PhaseOne_ArchiveFiles/Example%20Documentation/demographics_annon_2020-07-30.xlsx). Note that, the demographics file was updated progressively (as participants were often involved in multiple workshops), so the most up-to-date file was uploaded with each batch of claims.

While for file types 1-3, we initially uploaded one file per claim (for the SIPS2019 workshop batch), however given the time required to upload each file we shifted to preparing one file per batch of claims).[^3]

 Following this process, 12 batches of SCORE claims were analysed during Phase 1:
...
Depending on the stage of the data-processing pipeline development, these file required additional manual preparation prior to uploading into NVivo. This included, removing any #NA and ‘test’ listings, and removing duplicate listing (for example, when participants had multiple display names over the course of the project).

## Data Management

During Phase 1, data management was primarily through [NVivo](https://www.qsrinternational.com/nvivo-qualitative-data-analysis-software/home) (version 12) - a Computer-Aided Qualitative Data Analysis Software (CAQDAS) available via a University of Melbourne licence.

For each batch of claims, an NVivo Project was created with the relevant files processed and uploaded.[^4] Following recommendations in Woolf and Silver (2017, 108), for each NVivo Project a process of distribution-merge-distribution was used. The recommended steps involved in this process can be described as follows (Woolf and Silver 2017, 110):
...
There were two main types of tasks completed in NVivo subprojects:

1. reviewing each file type 1 to read the textual responses, interpreting them within the relevant context and applying the relevant 'nodes' from the codebook of analytic categories within NVivo. (See 'the Qualitative Analaysis section for more detail).

2. reviewing one or more of the other file types (2-4) and applying descriptive indexing of associated contextual data using the 'case classifications' function in NVivo.

In NVivo, case classifications function as descriptive indexing (rather than the analytic categorising that node coding is used for). This process of classifying multiple units of text at a given case allows for further analysis of the analytic coding. Fore example, it allows queries that reveal all responses classified to a given analytic category (nodes) across multiple claims for a given group, all all analytic categories associated to responses provided by particular group across multiple claims, etc.,). For Phase 1, we included two key case classifications:

- Claim attributes: linking the information in the claim metadata file columns to responses in the Justification_Comments files with corresponding ‘claim ids’.
- Rating attribute: linking the information in the the Ratings_NumericalResponses_by_Claim file to responses based on the ‘unique id participant’ column from each of the Justification_Comments files.
- Participant attributes: linking the information from the demographics file to responses via the participant id in the ‘justifier’ column from each of the Justification_Comments files.

These classifications were applied manually in NVivo and, given the workflow in this project, this was a separate process to the use of 'nodes' to apply analytic categories to text units. However, once both case classifications and nodes were applied to text-units, queries could be run within NVivo to extract subsets of data relevant to specific research questions.
...
We developed a codebook that specifies the key analytic categories used to interpret the textual data collected during the repliCATS project, and details the inclusion and exclusion criteria. The development of these analytic categories, and the iterative use of relevant versions of the codebook collecting these categories, can be summarised in terms of qualitative analytic task types (adapted from Woolf and Silver (2017).

| Analytic Task Type  | Task Description  | Summary of Task during Phase   | Associated Phase 1 Documentation |
|- |- |- |- |
...
Codebook [version 10](https://github.com/metamelb-repliCATS/RAT/blob/drafts/Archives%20RAT/PhaseOne_ArchiveFiles/Example%20Documentation/Codebook_V10_Details.xlsx) was considered a stable version of our main set of analytic categories for the purpose of Phase 1 (of these a subset use selected as ['reasonWAgg categories'](https://github.com/metamelb-repliCATS/RAT/blob/drafts/Archives%20RAT/PhaseOne_ArchiveFiles/Example%20Documentation/reasonWAggCategories_SummaryV5.docx)).
...
Following [training](https://github.com/metamelb-repliCATS/RAT/blob/drafts/Archives%20RAT/PhaseOne_ArchiveFiles/Example%20Documentation/TrainingMaterials_RAT_Phase1.docx), each analyst used a copy of the base NVivo project to read an assigned set of text-units, interpete these as responses to the elicitation questions within any available context, and index each responses to the applicable nodes from the relevant codebook.
...
These [reasonWAgg Categories](https://github.com/metamelb-repliCATS/RAT/blob/drafts/Archives%20RAT/PhaseOne_ArchiveFiles/Example%20Documentation/reasonWAggCategories_SummaryV5.docx) included 25 nodes [selected from the main Codebook](https://github.com/metamelb-repliCATS/RAT/blob/drafts/Archives%20RAT/PhaseOne_ArchiveFiles/Example%20Documentation/Reconciling_CodebookV10_with_reasonWAggCategoriesV5_Archived.xlsx).

Following the auto-code process, a query was run for each batch of claims, cross-referencing responses based on the reasonWAgg categories by participant per claim. The output of this query provides an indication of how many different categories of reasoning attributed to each participants based on their responses for any given claim. These query outputs were combined and provided to the Aggregation team, for the reasonWAgg aggregations - see [here](https://github.com/metamelb-repliCATS/aggreCAT) for more detail.
...
There are multiple approaches to calculating an ICR score, including calculations based on mean pairwise agreement, simultaneous agreement, and majority agreement (Popping 2010, 1071). Each of these approaches have specific benefits and limitations and different approaches and accepted levels of reliability are considered appropriate in specific contexts (Hruschka et al. 2004). In each case, an adequate ICR can usually be determined at the point when multiple analysts consistently allocated the same quotations to the same general group of codes (theme), even when not agreeing on each identical code for every unit of data (e.g., MacPhail et al. 2016, 206). For more detail see this [context for caluclating the ICR in qualitative analysis](https://github.com/metamelb-repliCATS/RAT/blob/master/Data%20analysis/Analyst_ICR_Calculations/ICR_Notes.md)
...
[^2]: Depending on the stage of the data-processing pipeline development, these file required additional manual preparation prior to uploading into NVivo. This included, removing any #NA and ‘test’ listings, and removing duplicate listing (for example, when participants had multiple display names over the course of the project).

[^3]: In addition to the SCORE data, we also supported the analysis of a batch of claims collected at the SIPS2019 workshops from the groups assessing ‘known-outcome claims’ rather than SCORE claims. These responses were collected and processed in the same way that the SCORE claims were, with some adjustments. In terms of collection, five groups assessed the same twenty-five claims, each of which had a known 'replication outcome'. Therefore, for the file types an additional column was added in the Metadata file for the replication outcome of each claim. In addition, a fifth file type was also generated for the self-reported expertise of participants for each of the claim topics (one row, per participant, with columns for each claim).

[^4]: This approach emerged following difficulties we experienced with NVivo when trying to upload larger quantities of data (including slow processing times and losing case classification information when merging large sub-projects).
